...
public class HomeActivity extends ListActivity implements LoaderCallbacks<Cursor>, OnItemClickListener, OnLVCCheckChangedListener{
	...
	// Called when the user selects a contextual menu item
		...
		public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
			...
			case R.id.home_Context_Hisaab:
				startActivity(Helper
						.showHisaab(getApplicationContext(), Helper.TRIPS,
								Helper.ListToIntegerArray(aa.getAllChecked(),
										tripList), -1));
				mode.finish();
				return true;
			case R.id.home_Context_Share:
				...
				Uri uri = Uri.parse("file://" + Helper.exportToFile(Helper.ListToIntegerArray(aa.getAllChecked(),tripList)));
			...
			case R.id.home_Context_export:
				...
				Toast.makeText(getApplicationContext(), "Exported to" +Helper.exportToFile(Helper.ListToIntegerArray(aa.getAllChecked(),tripList)).getPath(), Toast.LENGTH_LONG).show();
			return true;
			default:
				return false;
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		aa = new ListItemAdapter(this, R.layout.listitemhome, tripList);
...
//		Toast.makeText(getApplicationContext(), path, Toast.LENGTH_LONG).show();
		...
		DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
		    ...
		    public void onClick(DialogInterface dialog, int which) {
		        ...
		        case DialogInterface.BUTTON_POSITIVE:
		        		Helper.importTrip(path, getApplicationContext());
		        		ReloadList();
		            break;
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		String projection[] = new String[] { DBinfo.TRIP_ID,
			...
			,
				...
				DBinfo.CLM_TRIP_THEME,DBinfo.CLM_TRIP_TYPE };
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		if(tripList.size()<=0)
		{
			newItem=new ListItemClass(ListItemClass.ID_AddNewItem,"Add New Trip","","Click here to add","",0,0,"");
			tripList.add(newItem);
		}
		aa.notifyDataSetChanged();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==RESULT_OK && requestCode==TRIP_IMPORT_INTENT)
		{
                ...
                {
                	Helper.importTrip(recievedFile,getApplicationContext());
                	ReloadList();
                }
                else
                {

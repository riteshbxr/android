...
public class TripActivity extends ListActivity implements LoaderCallbacks<Cursor>, OnItemClickListener, OnLVCCheckChangedListener{
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_contacts:
			...
			intent.putExtra(DBinfo.CLM_TRIP_GROUPID, groupId);
			intent.putExtra("source", "TripActivity");
			startActivity(intent);
			return true;
		case R.id.action_gotoResults:
			startActivity(Helper.showHisaab(getApplicationContext(), Helper.GROUPS, groupId));
			return true;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==RESULT_OK && requestCode==TRIP_IMPORT_INTENT)
		{
                ...
                {
                	new Helper.importTrip(recievedFile,this,groupId,true).execute();
                }
                else
                {
		...
		// Called when the user selects a contextual menu item
			...
			public boolean onActionItemClicked(final ActionMode mode, MenuItem item) {
				...
				//	Toast.makeText(getApplicationContext() ,"Finally deleted:"+aa.getAllChecked().size(),Toast.LENGTH_SHORT ).show();
					...
					mode.finish();
					setundo=new UndoOptions(lstTrips,positions, Helper.getUndoCallback(getContentResolver(), new ArrayList<ListItemClass>(tripList), aa,Helper.TRIPS));
					return true;
				...
				case R.id.home_Context_Hisaab:
					startActivity(Helper.showHisaab(getApplicationContext(), Helper.TRIPS,

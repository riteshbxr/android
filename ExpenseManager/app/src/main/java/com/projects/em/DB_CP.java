...
public class DB_CP extends ContentProvider {
	// TODO DBinfo CLass
		public class DBinfo {
			...
			public static final String DB_TABLE_GROUPMEMBERS = "groupmember";
			public static final String GROUPMEMBER_ID = "_groupmemberid";
	...
	private static class DBHelper extends SQLiteOpenHelper {
		...
		private static final String CREATE_GROUPMEMBERS = "create table "
				+ DBinfo.DB_TABLE_GROUPMEMBERS + " ( " + DBinfo.GROUPMEMBER_ID
				+ " integer primary key autoincrement, " + DBinfo.GROUP_ID
				+ " integer, " + DBinfo.MEMBER_ID + " integer);";
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL(CREATE_GROUPMEMBERS);
			
			ContentValues values = new ContentValues();
			values.put(DBinfo.CLM_GROUP_NAME, "Main List");
			values.put(DBinfo.CLM_GROUP_THEME, 0);
			values.put(DBinfo.CLM_GROUP_TYPE, 0);
			values.put(DBinfo.CLM_GROUP_DESC, "Default Group Auto Created");
		...
		if(oldVersion==5)// needed as Groups table has a wrong column name
		{
			//      Renaming _tripid in GROUPTABLE to _groupid 
			String query1="ALTER TABLE "
					+ DBinfo.DB_TABLE_GROUPS
					+" RENAME TO "
					+ "tmp_groups";
			String query23= "create table "
					+ DBinfo.DB_TABLE_GROUPS + " AS "
					+" SELECT "
					+ " _tripid AS "+ DBinfo.GROUP_ID	+ "," 
					+ DBinfo.CLM_GROUP_NAME	+ "," 
					+ DBinfo.CLM_GROUP_THEME	+ "," 
					+ DBinfo.CLM_GROUP_TYPE	+ ","
					+ DBinfo.CLM_GROUP_DESC
					+" FROM tmp_groups;";
			/*
			...
			String query3="INSERT INTO "
					...
					+ DBinfo.CLM_GROUP_DESC
					+" FROM tmp_groups;";
			*/String query4="DROP TABLE tmp_groups";
			db.execSQL(query1);
			db.execSQL(query23);
			//db.execSQL(query3);
			db.execSQL(query4);
			...
			case 4:
				...
				db.insert(DBinfo.DB_TABLE_GROUPS, null, values);
			case 5:
				//ONE GROUPid COLUMN RENAMED EARLIER IN THE SAME LEVEL
				db.execSQL(CREATE_GROUPMEMBERS);
				String query="INSERT INTO "
						+ DBinfo.DB_TABLE_GROUPMEMBERS+ " (" 
						+ DBinfo.GROUP_ID	+ "," 
						+ DBinfo.MEMBER_ID	+ ")"
						+ " SELECT DISTINCT "	
						+ DBinfo.CLM_TRIP_GROUPID	+ "," 
						+ DBinfo.MEMBER_ID 
						+" FROM "
						+ DBinfo.DB_TABLE_TRIPMEMBERS +","
						+ DBinfo.DB_TABLE_TRIPS 
						+" WHERE "
						+ DBinfo.DB_TABLE_TRIPMEMBERS +"."+DBinfo.TRIP_ID
						+ "=" +DBinfo.DB_TABLE_TRIPS+"."+DBinfo.TRIP_ID;
				db.execSQL(query);
			case 6:
				//for wrong entry issue only(not needed for new installs from 6)
				String query1="ALTER TABLE "
						+ DBinfo.DB_TABLE_GROUPS
						+" RENAME TO "
						+ "tmp_groups";
	...
	public static final Uri GROUPMEMBER_URI = Uri
			.parse("content://com.projects.em/groupmembers");
	...
	private static final int GROUPMEMBER_ALLROWS = 14;
	private static final int GROUPMEMBER_SINGLE_ROW = 15;
	...
	static {
		...
		uriMatcher.addURI("com.projects.em", "groupmembers", GROUPMEMBER_ALLROWS);
		uriMatcher.addURI("com.projects.em", "groupmembers/#",GROUPMEMBER_SINGLE_ROW);
	...
	public static ListItemClass getListItemClass(Uri uri, String where) {
		...
		case TRIPMEMBER_ALLROWS:
			...
			projection = new String[] { DBinfo.TRIPMEMBER_ID, DBinfo.TRIP_ID,
					DBinfo.MEMBER_ID };
			Cursor tripMembercursor = queryBuilder.query(db, projection, where, null,
					null, null, null);
			if (tripMembercursor.getCount() != 0) {
				tripMembercursor.moveToFirst();
				newItem = new ListItemClass(tripMembercursor.getInt(0),
						tripMembercursor.getInt(1),
						tripMembercursor.getInt(2));
				return newItem;
			}
			break;
		case GROUPMEMBER_SINGLE_ROW:
		case GROUPMEMBER_ALLROWS:
			queryBuilder.setTables(DBinfo.DB_TABLE_GROUPMEMBERS);
			projection = new String[] { DBinfo.GROUPMEMBER_ID, DBinfo.GROUP_ID,
					DBinfo.MEMBER_ID };
			Cursor groupMembercursor = queryBuilder.query(db, projection, where, null,
					null, null, null);
			if (groupMembercursor.getCount() != 0) {
				groupMembercursor.moveToFirst();
				newItem = new ListItemClass(groupMembercursor.getInt(0),
						groupMembercursor.getInt(1),
						groupMembercursor.getInt(2));
				return newItem;
			}
			break;
		}
	...
	public static ArrayList<ListItemClass> getListItemClassArray(Uri uri,
		...
		case TRIPMEMBER_ALLROWS:
			...
			Cursor tripMembercursor = queryBuilder.query(db, projection, where, null,
					null, null, null);
			int tripMemberTaskIndex[] = {
					tripMembercursor.getColumnIndexOrThrow(DBinfo.TRIPMEMBER_ID),
					tripMembercursor.getColumnIndexOrThrow(DBinfo.TRIP_ID),
					tripMembercursor.getColumnIndexOrThrow(DBinfo.MEMBER_ID) };
			while (tripMembercursor.moveToNext())
				newItems.add(new ListItemClass(
						tripMembercursor.getInt(tripMemberTaskIndex[0]),
						tripMembercursor.getInt(tripMemberTaskIndex[1]), 
						tripMembercursor.getInt(tripMemberTaskIndex[2])));
			break;
		case GROUPMEMBER_SINGLE_ROW:
		case GROUPMEMBER_ALLROWS:
			queryBuilder.setTables(DBinfo.DB_TABLE_GROUPMEMBERS);
			...
			Cursor groupMembercursor = queryBuilder.query(db, projection, where, null,
					null, null, null);
			int groupMemberTaskIndex[] = {
					groupMembercursor.getColumnIndexOrThrow(DBinfo.GROUPMEMBER_ID),
					groupMembercursor.getColumnIndexOrThrow(DBinfo.GROUP_ID),
					groupMembercursor.getColumnIndexOrThrow(DBinfo.MEMBER_ID) };
			while (groupMembercursor.moveToNext())
				newItems.add(new ListItemClass(
						groupMembercursor.getInt(groupMemberTaskIndex[0]),
						groupMembercursor.getInt(groupMemberTaskIndex[1]), 
						groupMembercursor.getInt(groupMemberTaskIndex[2])));
			break;
		}
		return newItems;
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case GROUPMEMBER_SINGLE_ROW:
			rowID = uri.getPathSegments().get(1);
			selection = DBinfo.GROUPMEMBER_ID
					+ "="
					+ rowID
					+ (!TextUtils.isEmpty(selection) ? " AND (" + selection
							+ ')' : "");
		case GROUPMEMBER_ALLROWS:
			if (selection == null)
				selection = "1";
			deleteCount = db.delete(DBinfo.DB_TABLE_GROUPMEMBERS, selection,
					selectionArgs);
			break;
		}
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case GROUPMEMBER_SINGLE_ROW:
		case GROUPMEMBER_ALLROWS:
			id = db.insert(DBinfo.DB_TABLE_GROUPMEMBERS, nullColumnHack, values);
			break;
		}
	...
	if (sortOrder == null)
		...
		case GROUPMEMBER_SINGLE_ROW:
			rowID = uri.getPathSegments().get(1);
			queryBuilder.appendWhere(DBinfo.GROUPMEMBER_ID + "=" + rowID);
		case GROUPMEMBER_ALLROWS:
			queryBuilder.setTables(DBinfo.DB_TABLE_GROUPMEMBERS);
			break;
		case RAW:
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case TRIPMEMBER_ALLROWS:
			...
			break;
		case GROUPMEMBER_SINGLE_ROW:
			rowID = uri.getPathSegments().get(GROUPMEMBER_SINGLE_ROW);
			selection = DBinfo.GROUPMEMBER_ID
					+ "="
					+ rowID
					+ (!TextUtils.isEmpty(selection) ? " AND (" + selection
							+ ')' : "");
		case GROUPMEMBER_ALLROWS:
			updateCount = db.update(DBinfo.DB_TABLE_GROUPMEMBERS, values,
					selection, selectionArgs);// Perform the update.
			break;
		}
		// Notify any observers of the change in the data set.
	...
	public static int[] getAllTripIds() {
		return getAllTripIds(" 1 ");
	}
	public static int[] getAllTripIds(String where) {
		// TODO Auto-generated method stub
		int[] tripIds;
		List<ListItemClass> AllTrips =getListItemClassArray(TRIP_URI, where);
